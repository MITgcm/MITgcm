module OAD_auxillary_cp
  ...
  public :: cp_aux_isfwd, cp_aux_currtimestep
  integer :: cp_aux_isfwd, cp_aux_currtimestep
  ...
  subroutine findunit_i()
    ...
    do ii=9,999
       if (cp_io_unit.eq.-1) then
          ...
          if (ios.ne.0) then
             write(msgbuf,'(a,i2.2)')  'OAD_auxillary_cp:findunit_i: inquiring unit number = ',ii
             print *, msgBuf
             write(msgbuf,'(a)') 'OAD_auxillary_cp:findunit_i: inquire statement failed!'
             print *, msgBuf
             stop 'ABNORMAL END: S/R OAD_auxillary_cp:findunit_i'
          endif
          if (.NOT. op) then
             cp_io_unit=ii
          end if
    ...
    if (cp_io_unit.eq.-1) then
       write(msgbuf,'(a)')  'OAD_auxillary_cp:findunit_i: could not find an available unit number!'
       print *, msgBuf
       stop 'ABNORMAL END: S/R OAD_auxillary_cp:findunit_i'
    endif
  end subroutine
