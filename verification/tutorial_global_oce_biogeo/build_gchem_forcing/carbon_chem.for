...
C Millero p.664 (1995) using Mehrbach et al. data
C Both on seawater pH scale
           ak1(i,j,bi,bj)=10**(-1D0*(3670.7D0*invtk -
     &          62.008D0 + 9.7944D0*dlogtk -
     &          0.0118D0 * s + 0.000116D0*s2))
           ak2(i,j,bi,bj)=10**(-1*(1394.7D0*invtk + 4.777D0 -
     &          0.0184D0*s + 0.000118D0*s2))
...
c pressc = pressure in bars
     ...
     &         exp( (24.2D0-0.085D0*t)
     &             *(pressc-1.0D0)/(83.143D0*tk) )
...
C  pH scale (everything else is the same).
           akw(i,j,bi,bj) = exp(-13847.26D0*invtk + 148.9652D0 -
     &          23.6521D0*dlogtk + (118.67D0*invtk
     &          - 5.977D0 + 1.0495D0 * dlogtk) *
     &          sqrts - 0.01615D0 * s)
C------------------------------------------------------------------------
C ks = [H][SO4]/[HSO4]
C dickson (1990, J. chem. Thermodynamics 22, 113)
...
C Note: converted here from mol/kg-H2O to mol/kg-SW
           aks(i,j,bi,bj)=exp(-4276.1D0*invtk + 141.328D0 -
     &          23.093D0*dlogtk +
     &          (-13856D0*invtk + 324.57D0 - 47.986D0*dlogtk)
     &          *sqrtis +
     &          (35474D0*invtk - 771.54D0 + 114.723D0*dlogtk)
     &          *is -
...
C Following Mucci (1983) - from Zeebe/Wolf-Gladrow equic.m
            tmpa1 = - 171.9065D0 - (0.077993D0*tk)
     &            + (2839.319D0/tk) + (71.595D0*log10(tk))
            tmpa2 = +(-0.77712D0 + (0.0028426D0*tk)
     &           + (178.34D0/tk) )*sqrts
            tmpa3 = -(0.07711D0*s) + (0.0041249D0*s15)
            logKspc = tmpa1 + tmpa2 + tmpa3
            Ksp_T_Calc = 10.0D0**logKspc
