...
C     maskFk    :: 2-D mask for vertical interface k (between level k-1 & k)
      ...
      Real*8 dTdz
      Real*8 tmp1k(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
...
C-    Just initialize to zero (not use anyway)
      ...
      DO k=1,Nr
       DO j=1-OLy,sNy+OLy
        DO i=1-OLx,sNx+OLx
         GM_PsiX(i,j,k,bi,bj)  = 0.D0
         GM_PsiY(i,j,k,bi,bj)  = 0.D0
        ENDDO
       ENDDO
      ENDDO
