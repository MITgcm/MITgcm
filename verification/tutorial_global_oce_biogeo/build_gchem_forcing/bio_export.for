...
C  i,j,k                  :: loop indices
      ...
      Real*8 lit     (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 kall, atten
      Real*8 nutlimit
      Real*8 tmppo4
...
c ice fraction needs to be taken into account
         lit(i,j)=lit(i,j)*(1.D0 - FIce(i,j,bi,bj))
        ENDIF
       ENDDO
      ENDDO
...
C$TAF STORE lit = bio_export
       DO j=jMin,jMax
        DO i=iMin,iMax
         ...
         atten = kall*drF(k)*hFacC(i,j,k,bi,bj)*.5D0
         if (k.gt.1) atten = atten
     &         +( kall*drF(k-1)*hFacC(i,j,k-1,bi,bj)*.5D0 )
...
C     performance drop on vector computers
          IF (lit(i,j).LT.0.D0.OR.lit(i,j).GT.350.D0) THEN
