...
C Solubility relation coefficients
      ...
      Real*8 pisvel(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
C local variables for carbon chem
      Real*8 surfalk(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 surfphos(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 surfsi(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 surftemp(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 surfsalt(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 surfdic(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
...
C Schmidt number is accounted for later
          pisvel(i,j) = 0.337D0 *wind(i,j,bi,bj)**2/3.6D5
...
C$TAF LOOP = parallel
        DO i=iMin,iMax
          IF ( maskC(i,j,kLev,bi,bj).NE.0.D0 ) THEN
