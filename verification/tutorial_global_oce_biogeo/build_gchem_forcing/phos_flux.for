...
c  zbase                  :: depth of bottom of current productive layer
      ...
      Real*8 zbase
      Real*8 bexport(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 reminFac
      Real*8 depth_l
      Real*8 flux_u (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 flux_l
CEOP
...
C--   topography), then remineralize in here
          IF (k.EQ.Nr) THEN
           pflux(i,j,k)=pflux(i,j,k)+BIOac(i,j,k)*(1.D0-DOPfraction)
          ELSEIF (hFacC(i,j,k+1,bi,bj).EQ.0.D0) THEN
           pflux(i,j,k)=pflux(i,j,k)+BIOac(i,j,k)*(1.D0-DOPfraction)
          ELSE
C- flux out of layer k
           bexport(i,j)=BIOac(i,j,k)*(1.D0-DOPfraction)
