...
C     pH_isLoaded(2) :: = T when   3-D   pH is loaded from pickup file
     ...
     &              useCalciteSaturation, pH_isLoaded
      ...
      Real*8  AtmosP(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  pCO2(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  pH(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  FluxCO2(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  wind(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  fIce(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  Kwexch_Pre(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  silicaSurf(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  calciteDissolRate(2), calciteDissolExp(2)
      Real*8  calcOmegaCalciteFreq
      Real*8  zca
      Real*8  WsinkPIC
...
C   [Following Mucci (1983) with pressure dependence from Ingle (1975)]
     ...
     &                     ff,ft,st,bt, Ksp_TP_Calc
      ...
      Real*8  ak0(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  ak1(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  ak2(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  akw(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  akb(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  aks(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  akf(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  ak1p(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  ak2p(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  ak3p(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  aksi(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  ff(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
C Fugacity Factor added by Val Bennington Nov. 2010
      Real*8  fugf(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  ft(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  st(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  bt(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  Ksp_TP_Calc(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C Store dissociation coefficients for O2 solubility
     ...
     &              oB0,oB1,oB2,oB3,
     &              oC0
      Real*8 oA0,oA1,oA2,oA3,oA4,oA5
      Real*8 oB0,oB1,oB2,oB3
      Real*8 oC0
...
C  dic_pCO2          :: atmospheric pCO2 to be read from data.dic
     ...
     &        dic_pCO2, dic_int1, dic_int2, dic_int3, dic_int4
      ...
      Real*8     DIC_forcingPeriod
      Real*8     DIC_forcingCycle
      Real*8     dic_pCO2
      INTEGER dic_int1
      INTEGER dic_int2
      INTEGER dic_int3
...
c    &     calpha, crain_ratio, cInputFe, calpfe, feload, cfeload,
      ...
      Real*8 par(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 alpha(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 rain_ratio(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 InputFe(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 CHL(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 Kpo4
      Real*8 DOPfraction
      Real*8 zcrit
      Real*8 KRemin
      ...
      Real*8 KFe
      Real*8 freefemax
      Real*8 k0, kchl, parfrac, lit0
      Real*8 alphaUniform
      Real*8 rainRatioUniform
c     Real*8 alphamax, alphamin
c     Real*8 calpha
c     Real*8 crain_ratio
c     Real*8 cInputFe(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
c     Real*8 calpfe
c     Real*8 cfeload
c     Real*8 feload(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C  myThid               :: thread number
      Real*8  PTR_DIC(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  PTR_ALK(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  PTR_PO4(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  PTR_DOP(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)

      Real*8  PTR_O2(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
...
C  freefe                 :: iron not bound to ligand
      Real*8  GDIC(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  GALK(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  GPO4(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  GDOP(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  SURA(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8  SURC(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8  SURO(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8  CAR(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  BIOac(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  RDOP(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  pflux(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  exportflux(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  CAR_S(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8  cflux(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)

      Real*8  GO2(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
       ...
       DO k=1,Nr
         DO j=jMin,jMax
          DO i=iMin,iMax
           ...
           GALK(i,j,k)=+2.D0 *car(i,j,k)-R_NP*GPO4(i,j,k)
