...
CEOP
      ...
      solar = 1360.D0   !solar constant
      albedo= 0.6D0     !planetary albedo
...
C find day (****NOTE for year starting in winter*****)
     ...
     &                    /(360.D0*86400.D0)  !fraction of year
        yday = 2.D0*PI*dayfrac                    !convert to radians
        delta = (0.006918D0
     &         -(0.399912D0*cos(yday))            !cosine zenith angle
     &         +(0.070257D0*sin(yday))            !(paltridge+platt)
     &         -(0.006758D0*cos(2.D0*yday))
     &         +(0.000907D0*sin(2.D0*yday))
     &         -(0.002697D0*cos(3.D0*yday))
     &         +(0.001480D0*sin(3.D0*yday)) )
       DO j=1-OLy,sNy+OLy
...
C     (makes latitude independent of grid)
     ...
     &         lat = asin( fCori(1,j,1,bj)/(2.D0*omega) )
...
C daily average photosynthetically active solar radiation just below surface
          fluxi = solar*(1.D0-albedo)*cosz*frac*parfrac

C convert to sfac
          sfac(j) = MAX(1.D-5,fluxi)
       ENDDO !j
