...
C      Mix-Layer Eddies contribution to Bolus Transport in X dir.
     ...
     &                 *2.D0/(fcorLoc(i-1,j)+fcorLoc(i,j))
       ...
       DO k=2,Nr
        DO j=2-OLy,sNy+OLy-1
         DO i=3-OLx,sNx+OLx-1
          IF ( mixLayerU(i,j).GT.0.D0 ) THEN
            z2H = 2.D0*rF(k)/mixLayerU(i,j)
          ELSE
            z2H = 0.D0
          ENDIF
          mu_z = ( z2H + 1.D0 )*( z2H + 1.D0 )
          mu_z = ( 1.D0 - mu_z )*(1.D0 + mu_z*five_ov21 )
          mu_z = MAX( 0.D0, mu_z )
...
C      Mix-Layer Eddies contribution to Bolus Transport in Y dir.
     ...
     &                 *2.D0/(fcorLoc(i,j-1)+fcorLoc(i,j))
       ...
       DO k=2,Nr
        DO j=3-OLy,sNy+OLy-1
         DO i=2-OLx,sNx+OLx-1
          IF ( mixLayerV(i,j).GT.0.D0 ) THEN
            z2H = 2.D0*rF(k)/mixLayerV(i,j)
          ELSE
            z2H = 0.D0
          ENDIF
          mu_z = ( z2H + 1.D0 )*( z2H + 1.D0 )
