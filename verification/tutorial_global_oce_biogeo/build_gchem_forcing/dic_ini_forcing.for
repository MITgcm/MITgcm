...
C     chlinput      :: chlorophyll climatology input field [mg/m3]
     ...
     &    silicaSurf0, silicaSurf1
      ...
      Real*8 dicwind0   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 dicwind1   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      ...
      Real*8 silicaSurf0(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 silicaSurf1(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C-    Initialise DIC_VARS.h forcing-field arrays locally
       ...
       DO bi = myBxLo(myThid), myBxHi(myThid)
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
           wind      (i,j,bi,bj) = 0.D0
           fIce      (i,j,bi,bj) = 0.D0
           AtmosP    (i,j,bi,bj) = 0.D0
           silicaSurf(i,j,bi,bj) = 0.D0
...
C          wind(i,j,bi,bj)       = 5.D0*maskC(i,j,1,bi,bj)
C          AtmosP(i,j,bi,bj)     = 1.D0*maskC(i,j,1,bi,bj)
C          silicaSurf(i,j,bi,bj) = 7.6838D-3*maskC(i,j,1,bi,bj)
C          fIce(i,j,bi,bj)       = 0.D0
C#ifdef READ_PAR
C          par(i,j,bi,bj)        = 100.D0*maskC(i,j,1,bi,bj)
C#endif
C#ifdef LIGHT_CHL
CC If the chlorophyll climatology is not provided, use this default value.
C          CHL(i,j,bi,bj)        = 1.D-2*maskC(i,j,1,bi,bj)
C#endif
C#ifdef ALLOW_FE
C         InputFe(i,j,bi,bj)    = 1.D-11*maskC(i,j,1,bi,bj)
C#endif

         ENDDO
        ENDDO 

CC-    Initialise DIC_LOAD.h arrays:
        DIC_ldRec(bi,bj) = 0
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
          dicwind0(i,j,bi,bj) = 0.D0
          dicwind1(i,j,bi,bj) = 0.D0
          atmosp0 (i,j,bi,bj) = 0.D0
          atmosp1 (i,j,bi,bj) = 0.D0
          silicaSurf0(i,j,bi,bj) = 0.D0
          silicaSurf1(i,j,bi,bj) = 0.D0
          ice0    (i,j,bi,bj) = 0.D0
          ice1    (i,j,bi,bj) = 0.D0
...
Cc#ifdef READ_PAR
     ...
     &                        1, nIter0, myThid )
        CALL EXCH_XY_RL (  par,  myThid  )
      ENDIF
Cc#endif
Cc#ifdef LIGHT_CHL
CC--   Load chlorophyll climatology data, unit for chlorophyll : mg/m3
      IF ( DIC_chlaFile .NE. ' '  ) THEN
        CALL READ_REC_XY_RL( DIC_chlaFile, CHL,
     &                        1, nIter0, myThid )
        CALL EXCH_XY_RL (  CHL,  myThid  )
      ENDIF
Cc#endif
Cc#ifdef ALLOW_FE
