...
C   total_atmos_moles :: atmosphere total gas content (should be parameter)
      Real*8 total_atmos_moles
      INTEGER bi, bj, i,j,k
      INTEGER ntim

      Real*8 tile_flux  (nSx,nSy)
      Real*8 tile_carbon(nSx,nSy)
      Real*8 total_flux
      Real*8 total_carbon

C for carbon budget ouput
      INTEGER ioUnit
      Real*8 total_ocean_carbon_old
      Real*8 total_atmos_carbon_old
      Real*8 total_carbon_old, carbon_diff
      Real*8 year_diff_ocean, year_diff_atmos, year_total
      Real*8 start_diff_ocean, start_diff_atmos, start_total
...
C             dic_int4=timestep between file entries)
        ...
        aWght = 0.5D0 + aWght/bWght - FLOAT(ntim-1)
