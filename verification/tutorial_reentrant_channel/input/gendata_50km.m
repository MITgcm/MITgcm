...
r1 = bump_max*repmat(sin(0:pi/8:pi),[ny 1])'; % sinusoidal bump running N-S through middle of domain
r2 = 0:.2:1;                                  % create linear ramp for center notch
bump(6:14,:) = r1;
bump(6:14,14:19) = bump(6:14,14:19).*fliplr(repmat(r2,[9 1])); % linearly lower bump height toward center notch
bump(6:14,21:26) = bump(6:14,21:26).*repmat(r2,[9 1]);         % linearly lower bump height toward center notch
bump(6:14,19:21) = 0.;     % notch; in these latitude bands, contours of f/H are unblocked
bathy = bathy + bump;
...
fid=fopen('T_relax_mask.50km.bin','w','b');fwrite(fid,rbcs_mask,'float32');fclose(fid);
...
fid=fopen('SST_relax.50km.bin','w','b');fwrite(fid,sst_relax,'float32');fclose(fid);
