...
r1 = bump_max*np.sin(np.linspace(0,np.pi,9)) # sinusoidal bump running N-S through middle of domain
r2 = np.reshape((np.linspace(0,1,6)),[6,1])  # create linear ramp for center notch
bump[:,5:14] = r1;
bump[13:19,:] = bump[13:19,:]*np.flip(r2[:]) # linearly lower bump height toward center notch
bump[20:26,:] = bump[20:26,:]*r2[:]          # linearly lower bump height toward center notch
...
taux[:,:] = np.reshape(taux_max*np.sin(np.linspace(0,np.pi,ny)),[ny,1])
...
Tmax = 10.
Tmin = -2.
sst_relax = np.zeros([ny,nx])+np.reshape(Tmin + (Tmax-Tmin)/40*np.linspace(0.5,39.5,ny),[ny,1]) # at (XC,YC) points
