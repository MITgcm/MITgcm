...
bump_max = 2000.
bathy = H*np.ones([ny,nx]); bump=np.zeros([ny,nx])
r1 = bump_max*np.sin(np.linspace(0,np.pi,80))    # sinusoidal bump running N-S through middle of domain
r2 = np.reshape((np.linspace(0,1,52)),[52,1])    # create linear ramp for center notch
bump[:,55:135] = r1;
bump[134:186,:] = bump[134:186,:]*np.flip(r2[:]) # linearly lower bump height toward center notch
bump[204:256,:] = bump[204:256,:]*r2[:]          # linearly lower bump height toward center notch
bump[185:205,:] = 0.0                            # notch; in these latitude bands, contours of f/H are unblocked
bathy = bathy+bump;
bathy[0:10,:] = 0.                               # wall at southern boundary
...
taux[5:395,:] = np.reshape(taux_max*np.sin(np.linspace(0,np.pi,390)),[390,1])
...
Tmax = 10.
Tmin = -2.
sst_relax = np.zeros([ny,nx])+np.reshape(Tmin + (Tmax-Tmin)/40*np.linspace(0.05,39.95,ny),[ny,1]) # at (XC,YC) points
