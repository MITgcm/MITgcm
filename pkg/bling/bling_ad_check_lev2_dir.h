#ifdef ALLOW_BLING

CADJ STORE pH                = tapelev2, key = ilev_2
CADJ STORE fice              = tapelev2, key = ilev_2
CADJ STORE atmosP            = tapelev2, key = ilev_2
CADJ STORE wind              = tapelev2, key = ilev_2
CADJ STORE silica            = tapelev2, key = ilev_2
CADJ STORE ak1               = tapelev2, key = ilev_2
CADJ STORE ak2               = tapelev2, key = ilev_2
CADJ STORE akf               = tapelev2, key = ilev_2
CADJ STORE ft                = tapelev2, key = ilev_2

CADJ STORE irr_mem           = tapelev2, key = ilev_2
CADJ STORE chl               = tapelev2, key = ilev_2
CADJ STORE poc               = tapelev2, key = ilev_2
CADJ STORE phyto_sm          = tapelev2, key = ilev_2
CADJ STORE phyto_lg          = tapelev2, key = ilev_2
# ifndef USE_BLING_V1
CADJ STORE phyto_diaz        = tapelev2, key = ilev_2
# endif

CADJ STORE dicwind0          = tapelvs2, key = ilev_2
CADJ STORE dicwind1          = tapelvs2, key = ilev_2
CADJ STORE atmosp0           = tapelvs2, key = ilev_2
CADJ STORE atmosp1           = tapelvs2, key = ilev_2
CADJ STORE silica0           = tapelvs2, key = ilev_2
CADJ STORE silica1           = tapelvs2, key = ilev_2
CADJ STORE ice0              = tapelvs2, key = ilev_2
CADJ STORE ice1              = tapelvs2, key = ilev_2
CADJ STORE feinput0          = tapelvs2, key = ilev_2
CADJ STORE feinput1          = tapelvs2, key = ilev_2

#ifdef ALLOW_EXF
CADJ STORE apressure         = tapelev2, key = ilev_2
CADJ STORE wspeed            = tapelev2, key = ilev_2
CADJ STORE apco20            = tapelev2, key = ilev_2
CADJ STORE apco21            = tapelev2, key = ilev_2
# endif

#endif /* ALLOW_BLING */
