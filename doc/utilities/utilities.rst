...
`utils/scripts` contains utilities for handling model input and output. You can 
...
.. code-block:: sh
   ...
   gluemnc <input_filename>.nc

Options:
""""""""

- **`<input_filename>`**: A pattern matching the tiled NetCDF files to merge. 
...
This will generate a single `state.0000000000.glob.nc` file containing the full 
...
- `ncks` installed from `NetCDF Operators (NCO) <http://nco.sourceforge.net>`_. 
- `utils/scripts/xplodemnc` in a findable location, either by adding it to the
  system search path or by copy+pasting the script in your working directory. 
