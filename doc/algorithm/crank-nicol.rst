...
.. math::
     ...
     \\
     {\eta}^* & = &
     \epsilon_{fs} {\eta}^{n} + \epsilon_{fw} \Delta t ({\cal{P-E}})
     - \Delta t {\bf \nabla}_h \cdot \int_{R_{fixed}}^{R_o}
...
#. The RHS term of equation :eq:`eta-n+1-CrankNick` corresponds the
   contribution of fresh water flux ({\cal{P-E}}) to the free-surface variations
   ...
   :math:`\epsilon_{fw} ( \beta ({\cal{P-E}})^{n+1/2} + (1-\beta) ({\cal{P-E}})^{n-1/2} )`
